...
class SvgToNextionConverter {
    ...
    private fun convertSVGToPNGBuffered(svgFilePath: String): BufferedImage {
        ...
        pngTranscoder.addTranscodingHint(SVGAbstractTranscoder.KEY_HEIGHT, 40f)
        pngTranscoder.addTranscodingHint(SVGAbstractTranscoder.KEY_WIDTH, 40f)
    ...
    fun convert(svgFilePath: String, debug: Boolean = false): List<Point> {
        val result=ArrayList<Point>()
        val image = convertSVGToPNGBuffered(svgFilePath)
        if (debug) {
